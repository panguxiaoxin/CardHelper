...
class ScanUtil {
    ...
    static final PROXY_CLASS_PREFIX = "CardManager\$\$Root\$\$"
    static final PROXY_CLASS_SUFFIX = "\$\$Proxy.class"
    static final PROXY_CLASS_PACKAGE_NAME = "com/jianxin/android/card/cards"

    static final REGISTER_CLASS_FILE_NAME = "com/ryx/card_api/core/CardManager.class"
    ...
    static boolean isTargetProxyClass(File file) {
        if (file.name.startsWith(PROXY_CLASS_PREFIX)) {
            return true
        }
        return false
    }
    ...
    static List<String> scanJar(File jarFile, File destFile) {
        ...
        while (enumeration.hasMoreElements()) {
            ...
            if (entryName == REGISTER_CLASS_FILE_NAME) {
                //标记这个jar包包含 CardManager.class
                //扫描结束后，我们会生成注册代码到这个文件里
                FILE_CONTAINS_INIT_CLASS = destFile
            } else {
                if (entryName.startsWith(PROXY_CLASS_PACKAGE_NAME+"/"+PROXY_CLASS_PREFIX)) {
                    if (list == null) {
                        list = new ArrayList<>()
                    }
